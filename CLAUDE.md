...
- `infra/cli/` — Python uv-workspace member named `macosworld-usecomputer`. The benchmark harness. Entry: `python -m benchmark.cli`.
...
- Python: always use `uv` from the repo root. Workspace members are listed in root `pyproject.toml`.
  - Run the CLI: `uv run --package macosworld-usecomputer python -m benchmark.cli ...`
  - Or via the justfile: `just bench <model> <tasks>`.
